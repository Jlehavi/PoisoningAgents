...
client = OpenAI(api_key="--------------------------")  # or hard-code for quick tests, but not recommended.
...
# Use only the highway_env rendering, don't create a separate matplotlib figure
plt.ion()
...
def predict_action(observation):
    ...
    # Only log to file, don't print to terminal to speed up execution
...
while not (done or truncated):
    ...
    # Display the render from highway-env
    frame = env.render()
    ...
    plt.axis('off')
    plt.draw()
    plt.pause(0.01)  # Reduced pause time for faster simulation
    plt.close()  # Close the figure after pausing to avoid accumulating figures
