...
def compute_avg_cluster_distance(query_embedding, cluster_centers):
    ...
    """
    # Ensure query_embedding and cluster_centers have the same dimension
    if query_embedding.shape[-1] != cluster_centers.shape[-1]:
        # If dimensions don't match, use the minimum dimension
        min_dim = min(query_embedding.shape[-1], cluster_centers.shape[-1])
        query_embedding = query_embedding[..., :min_dim]
        cluster_centers = cluster_centers[..., :min_dim]
...
class GradientStorage:
    ...
    def hook(self, module, grad_in, grad_out):
        # Handle DPR output which returns DPRContextEncoderOutput
        if hasattr(grad_out[0], 'pooler_output'):
            grad_tensor = grad_out[0].pooler_output
        else:
            grad_tensor = grad_out[0]
            
        if self._stored_gradient is None:
            self._stored_gradient = grad_tensor[:, -self.num_adv_passage_tokens:]
        else:
            self._stored_gradient += grad_tensor[:, -self.num_adv_passage_tokens:]
...
if __name__ == "__main__":
    ...
    with open(f"{root_dir}/stdout.txt", "w") as f:
        ...
        if not args.golden_trigger:
            ...
            elif args.agent == "MultiAgentPoison" or args.agent == "myagent":
                initial_trigger = "Be careful and drive safe."
        ...
        if args.agent == "ad":
            ...
            CoT_prefix = "You are a helpful assistant for highway intersection navigation.\n"
        ...
        if args.agent == "ad":
            ...
            split_ratio = 0.5
            test_samples_dir = "EhrAgent/database/eicu_ac.json"
            ...
            database_samples_dir = "MyAgent/result/MyAgentPoison"
            db_dir = "MyAgent/result/MyAgentPoison"
            # Load the database embeddings
            db_embeddings = load_db_multiagent(database_samples_dir, db_dir, model_code, model, tokenizer, device)
            split_ratio = 0.5
            test_samples_dir = "MyAgent/result/MyAgentPoison/results.json"
            train_dataset = MultiAgentPoisonDataset(test_samples_dir, split_ratio=split_ratio, train=True)
            valid_dataset = MultiAgentPoisonDataset(test_samples_dir, split_ratio=split_ratio, train=False)
            slice = 0
        ...
        # Load data
        if args.agent == "MultiAgentPoison" or args.agent == "myagent":
            # Load results from the results.json file
            with open("MyAgent/result/MyAgentPoison/results.json", "r") as f:
                results = json.load(f)
            ...
            for result in results:
                data_point = {
                    "Ego": f"Episode {result['episode']}, Step {result['steps']}, Total Reward {result['total_reward']:.2f}",
                    "Perception": f"Memory Size {result['memory_size']}, Agent Rewards {result['agent_rewards']}",
                    "Action": "",
                    "Reward": result['total_reward']
                }
                all_data.append(data_point)
        else:
            all_data = load_data(args.test_samples_dir, args.agent)

        # Initialize trigger sequence
        if args.agent == "MultiAgentPoison" or args.agent == "myagent":
            trigger_sequence = "Be careful and drive safe."
        else:
            trigger_sequence = "Be careful and drive safe."
        ...
        for it_ in range(args.num_iter):
            ...
            for _ in pbar:
                ...
                if args.agent == "ad" or args.agent == "qa" or args.agent == "MultiAgentPoison" or args.agent == "myagent":
            ...
            if (candidate_scores > current_score).any(): #or (candidate_acc_rates > current_acc_rate).any():
                ...
                if not target_gradient_guidance or args.agent == "MultiAgentPoison":
